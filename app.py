...
# === Map Info Functions ===
async def get_map_info(map_code: str) -> dict:
    """Fetch map information based on map code"""
    # Create a MapInfo protobuf message
    map_info = MapInfo()
    map_info.map_code = map_code
    map_info.map_title = f"Map {map_code}"
    map_info.description = f"This is a description for map {map_code}"
...
@app.route('/map-info')
@cached_endpoint()
def get_map_info_route():
    map_code = request.args.get('map_code')
    
    if not map_code:
        return jsonify({"error": "Please provide map_code."}), 400
    
    try:
        map_data = asyncio.run(get_map_info(map_code))
